...
fun MainScreen(
    ...
    val irisPresent = FileUtils.fileExists("/sys/kernel/iris/chip_version")
    ...
    var memcGame by remember { mutableStateOf(MemcGameService.isEnabled(context)) }
    var memcVideo by remember { mutableStateOf(VideoMemcService.isMasterEnabled(context)) }
    ...
    OnResume {
        ...
        if (irisPresent) {
            memcGame = MemcGameService.isEnabled(context)
            memcVideo = VideoMemcService.isMasterEnabled(context)
        }
    ...
    SettingsScroll(contentPadding) {
        ...
        if (irisPresent) {
            PreferenceGroup(title = stringResource(R.string.video_category_title)) {
                item {
                    SwitchPreference(
                        title = stringResource(R.string.memc_video_title),
                        summary = stringResource(R.string.memc_video_summary),
                        checked = memcVideo,
                        customIcon = { PrefIcon(R.drawable.ic_video) },
                        onCheckedChange = { enable ->
                            VideoMemcService.setMasterEnabled(context, enable)
                            memcVideo = enable
                            Log.i(TAG, "Video MEMC " + if (enable) "enabled" else "disabled")
                        },
                    )
                }
            }
        ...
        PreferenceGroup(title = stringResource(R.string.gaming_category_title)) {
            if (irisPresent) {
                item {
                    SwitchPreference(
                        title = stringResource(R.string.memc_game_title),
                        summary = stringResource(R.string.memc_game_summary),
                        checked = memcGame,
                        customIcon = { PrefIcon(R.drawable.ic_memc) },
                        onCheckedChange = { enable ->
                            MemcGameService.setEnabled(context, enable)
                            MemcGameService.notifyStateChanged(context)
                            memcGame = enable
                            Log.i(TAG, "Game MEMC " + if (enable) "enabled" else "disabled")
                        },
                    )
                }
                item {
                    ClickablePreference(
                        title = stringResource(R.string.memc_game_apps_title),
                        summary = stringResource(R.string.memc_game_apps_summary),
                        enabled = memcGame,
                        customIcon = { PrefIcon(R.drawable.ic_apps) },
                        onClick = onOpenMemc,
                    )
                }
            }
